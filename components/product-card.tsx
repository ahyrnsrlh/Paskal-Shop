  ...
  return (
    <Card className="overflow-hidden border-0 shadow-lg hover:shadow-xl transition-shadow duration-300 bg-white rounded-xl">
      ...
      <CardContent className="p-6">
        <h3 className="font-semibold text-lg mb-3 line-clamp-2 text-gray-900">{name}</h3>
        <p className="text-2xl font-bold text-green-600 mb-2">{formatPrice(price)}</p>
        <p className="text-sm text-gray-500">Stok: {stock}</p>
      </CardContent>
      <CardFooter className="p-6 pt-0 flex gap-3">
        <Button asChild variant="outline" className="flex-1 border-gray-200 text-gray-700 hover:bg-gray-50 rounded-lg">
          <Link href={`/products/${id}`}>Detail</Link>
        </Button>
        <Button asChild className="flex-1 bg-green-600 hover:bg-green-700 rounded-lg">
